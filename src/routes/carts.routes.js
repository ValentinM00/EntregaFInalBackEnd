...
router.get("/:cid", async (req, res) => {
    try{
        ...
        const cart = await cartDao.getById(Number(cid));
...
router.post("/:cid/product/:pid", async (req, res) => {
    try {
        ...
        res.status(200).json({ status: "success", payloaded: cartUpdate });
    }catch (error) {
        console.log(error);
        res.status(500).json({ status: "Error", msg: "Error interno del servidor"});
    }
...
router.delete("/:cid/product/:pid", async (req, res) => {
    try {
        ...
        console.log(error);
        res.status(500).json({ status: "Erorr", msg: "Error interno del servidor" });
    }
});
...
router.put("/:cid/product/:pid", async (req, res) => {
    try {
        ...
        const product = await productDao.getById(pid);
        if (!product) return res.status(404).json({ status: "Error", msg: `No se encontró el carrito con el id ${pid}` });
        const cart = await cartDao.getById(cid);
        if (!cart) return res.status(404).json({ status: "Error", msg: `No se encontró el carrito con el id ${cid}` });
